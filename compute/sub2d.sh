...
SEED=1
RHO=0.5
LY=60 # half of Ly = 120σ (Ly = 30, 60, 100, 120, 140, 180σ), chosen to compare with Fig. S6

# Create timestamp variables for date and time
DATE=$(date +%y%m%d)
TIME=$(date +%H%M)

# Run the LAMMPS job for EPS values incrementing by 5 from 20 to 50
for EPS in {50..50..5}; do
  FILENAME="${DATE}.${TIME}.eps.${EPS}.ly.$((2*LY))"
  echo "Running with EPS=$EPS"
  mpirun --bind-to none -n 64 $HOME/mylammps/src/lmp_mpi -in abp2d-wet.lmp \
