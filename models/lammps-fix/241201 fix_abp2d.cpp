  ...
  Fix(lmp, narg, arg)
{
  ...
  translational_friction = utils::numeric(FLERR,arg[4],false,lmp); 

  translational_diff = t_target/translational_friction;

  rotational_diff = 3*translational_diff; //3D, assuming sigma=1.0

  active_velocity = utils::numeric(FLERR,arg[5],false,lmp); 


  seed = utils::inumeric(FLERR,arg[6],false,lmp) ; //seed for random number generator. integer
...
void FixABP2D::init()
{
 ...
 if (fabs(orientation[0][0])<1e-6 && fabs(orientation[0][1])<1e-6){
   ...
   for (int i = 0; i < nlocal; i++){
    // correction! should range from 0 to 2pi
    // phi=random->uniform()*M_PI;
    phi = random->uniform() * 2 * M_PI;
  
    orientation[i][0] = cos(phi);
    // correction! orientation[i][0] is assigned twice, which is likely a typo.
    // orientation[i][0] = sin(phi);
    orientation[i][1] = sin(phi);
    // should initialise if needed: orientation[i][2] = 0.0;  // For 2D, z-component is zero
